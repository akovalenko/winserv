/*
 WinServ2 -- an utility to make a service from any executable.

 (c) 2004-2005 Anton Kovalenko

 Do what you want with this software, but don't blame me.

 It can forward SCM requests to the script's stdin and read log messages
 from the script's stdout.

 It can also use named pipes winserv.scm.in.<svcname>
 and winserv.scm.out.<svcname> to communicate with the process.
...
WSLINKAGE int con_ftprintf(FILE * stream, LPCTSTR fmt, ... );


/* SECTION: generic re-usable functions to make programming C 
 * a little easier */


/* SUBSECTION: registry */

/* reg_read is a wrapper for RegQueryValueEx that takes care of 
 * malloc and realloc, and autoexpands REG_EXPAND_SZ values when 
 * requested.
 *
 * Non-obvious argument: val_ptr is a pointer to pointer,
 *	to make the caller's variable point at malloc'ed storage. 
 *
 * Returns: type of value
 */
static int do_nothing(FILE *fd,...) {return 0;}
#ifdef DBGLOGGING
...
reg_read(HKEY hkey, LPCTSTR vname, LPBYTE *val_ptr, BOOL autoexpand)
{
	...
	*val_ptr = realloc(*val_ptr, bfrSize);
	...
	free(*val_ptr); *val_ptr=NULL; return 0;
    }
	...
	*val_ptr = malloc(exp_cnt*sizeof(TCHAR));
	exp_rq = ExpandEnvironmentStrings(not_expanded,(LPTSTR)*val_ptr,exp_cnt);
	if (exp_rq > exp_cnt) {
	    *val_ptr = realloc(*val_ptr, exp_rq*sizeof(TCHAR));
	    ExpandEnvironmentStrings(not_expanded,(LPTSTR)*val_ptr,exp_rq);
	}
	free(not_expanded);
    }
    return vtype;
...
WSLINKAGE
HKEY reg_open(LPCTSTR machine, HKEY parent, LPCTSTR keypath, int mode)
{
	...
	return NULL;
    if (mode==O_CREAT)  {
	if (ERROR_SUCCESS!=RegCreateKeyEx(real_parent,keypath,0,NULL,
		REG_OPTION_NON_VOLATILE,sam,NULL,&result,NULL))
	    return NULL;
    } else {
	if (ERROR_SUCCESS!=RegOpenKeyEx(real_parent,keypath,0,sam,&result))
	    return NULL;
    }
...
/* Duplicate handle pointed by "what",
 * making it noninheritable. The resulting handle is 
 * stored in the place of old one */
WSLINKAGE
BOOL fh_dup_noinherit(HANDLE *what )
{
    HANDLE me,newh;
    BOOL result;

    me = GetCurrentProcess();
    result = DuplicateHandle(me, *what, me, &newh, 0, 
...
WSLINKAGE
BOOL fh_pipe(HANDLE *this_end, HANDLE *that_end, BOOL we_write)
{
	...
	eventlog_error(TEXT("WinServ (CreatePipe)"));
	return 0;
    }
}
...
WSLINKAGE
FILE* fh_to_stream(HANDLE fh, int for_open, const char *for_fopen)
{
    int crfh;
    FILE *stream;
    if (!fh) return NULL;
...
WSLINKAGE
HANDLE stream_to_fh(FILE* stream)
{
...
/* The routine that does fgets in a separate thread,
 * signalling when data is ready and then waiting for someone
 * to process the data */
static
DWORD WINAPI fh_async_fgets_thread(LPVOID clientData)
{
...
WSLINKAGE
async_fgets_t *fh_async_fgets_start(HANDLE hFile)
{
    async_fgets_t *afg;
...
WSLINKAGE
VOID fh_async_fgets_terminate(async_fgets_t *afg) { if (afg) {
 ...
 * in an array.
 * Use offsetof() macro to get the offset for a structure element
 * */
WSLINKAGE
void * str_field_lookup(LPCTSTR what, void *where, int itemsize, int off)
{
...
WSLINKAGE
LPTSTR str_vprintf(LPCTSTR fmt, va_list args)
{
    LPTSTR buf;
    size_t bsz = 64;
...
typedef struct {DWORD buf; LPTSTR msg, ptr; } str_msg_t;

WSLINKAGE
void str_msg_start(str_msg_t *mb, DWORD sz)
{
    mb->buf = sz;
...
WSLINKAGE
void str_msg_add(str_msg_t *mb, LPCTSTR fmt, ...) 
{
    int nch;
    va_list args;
    va_start(args,fmt);
...
WSLINKAGE
int con_ftprintf(FILE * stream, LPCTSTR fmt, ... )
{
    LPTSTR s;
    DWORD ncw;
    va_list args;
...
WSLINKAGE
DWORD lookup_kw(LPCTSTR what, keyword_t *kws)
{
    keyword_t *r;
    r = str_field_lookup(what,kws,sizeof(keyword_t),offsetof(keyword_t,kw));
    if (!r) return -1234;
    else return r->value;
}

WSLINKAGE
LPCTSTR which_kw(DWORD val, keyword_t *kws, LPCTSTR dflt)
{
    int i;
    for (i=0; kws[i].kw; ++i) {
	if(kws[i].value==val) {
...
static LPCTSTR cvt_fetch(int *pargc, LPCTSTR **pargv) 
{
    LPCTSTR r;
...
#define OD_defgroup(name) \
static option_t name[]={
...
static int cvtSTRING (option_t *opt, int *pargc, LPCTSTR **pargv)
{
    CVT_PROLOGUE;
    CVT_PUT(LPCTSTR,s);
...
static int cvtCHOICE (option_t *opt, int *pargc, LPCTSTR **pargv)
{
    ...
    CVT_PROLOGUE;
    if ((x=lookup_kw(s,variants))!=-1234) {
	CVT_PUT(DWORD,x);
	return 0;
    }
	...
	if (i==0)
	    ...
	    str_msg_add(&mb,variants[i+1].kw ? TEXT(", %s"):TEXT(" or %s"), 
		    variants[i].kw);
    }
...
static int cvtSTRINGTABLE (option_t *opt, int *pargc, LPCTSTR **pargv)
{
    LPTSTR stab;
    int i;
    CVT_PROLOGUE;
    ...
    for (i=0; s[i]; ++i) 
	switch (s[i]) {
	    case TEXT(','): stab[i]=TEXT('\000'); break;
	    default: stab[i]=s[i];
	}
    stab[i++]=TEXT('\000');
    stab[i]=TEXT('\000');
...
WSLINKAGE
DWORD parse_options(int *pargc, LPTSTR **pargv, option_t **opts)
{
    /**/
    option_t *theopt;
    int i;
	...
	for (i=0,theopt=NULL;opts[i];++i) 
	    if ((theopt=str_field_lookup(**pargv, opts[i],
			    sizeof(option_t),
			    offsetof(option_t,name))))
		break;
	if (!theopt) {
...
/*
 * SECTION: The Real Winserv
 * */

/* WinServ can communicate with an underlying process 
...
/* WinServ invokation:
 * winserv subcommand ?servicename? ?options?
 * */
...
WSLINKAGE 
DWORD ipc_method=IPCMETHOD_BLIND;
WSLINKAGE 
LPTSTR ipc_method_name=NULL;
WSLINKAGE
int is_in_service=0;

WSLINKAGE
BOOL pass_control(LPSERVICE_MAIN_FUNCTION main_fun);
WSLINKAGE
void WINAPI sv_main(DWORD dwArgc, LPTSTR *lpszArgv);
...
/* The main function will try to StartServiceCtrlDispatcher 
 * if no arguments is given. If there are some arguments, 
 * it calls util_main that implements WinServ command-line 
 * utility
 */
int _cdecl
#ifdef UNICODE
...
(int argc, LPTSTR *argv)
{
	...
	if(!pass_control(sv_main)) {
	    con_ftprintf(stderr,TEXT(
    "Start this program with ServiceStart(),\n\
...
WSLINKAGE void sv_set_args(DWORD argc, LPTSTR *argv)
{
    LPTSTR arg;
...
WSLINKAGE VOID eventlog_open() { eventlog = RegisterEventSource(NULL, sv_name); }
WSLINKAGE VOID eventlog_close() { DeregisterEventSource(eventlog); }
...
WSLINKAGE VOID eventlog_msg(int type, LPTSTR msg)
{
    ...
    strings[0]=msg;
    ReportEvent(eventlog, (short)type, 0,  1, NULL,
	    1,  0, (LPCTSTR*)strings, NULL); 
}
...
{
    BOOL ok=TRUE;
    LPTSTR path=str_printf(
	    TEXT("System\\CurrentControlSet\\Services\\%s\\WinServ"), sv_name);
    HKEY hkey=reg_open(NULL,HKEY_LOCAL_MACHINE,path,O_RDONLY);

    if (!hkey) goto error;
    if (reg_read(hkey,TEXT("CommandLine"),(LPBYTE*)&command_line,TRUE)!=REG_SZ)
	goto error;
...
WSLINKAGE BOOL start_process()
{
	 ...
	 * as we do nothing expect reporting SERVICE_STOPPED and exiting */
	eventlog_error(TEXT("WinServ (CreateProcess)"));
	return FALSE;
    }
...
WSLINKAGE BOOL main_loop()
{
    ...
    if ((ipc_write_stream = fh_to_stream(
		    ipc_write,O_WRONLY|O_TEXT,"wt")))
	setvbuf(ipc_write_stream,NULL,_IONBF,0);
    for (;;)
	switch (WaitForMultipleObjects(objc,objects,FALSE,INFINITE))
	{
	    case WAIT_OBJECT_0:
		...
		eventlog_msg(EVENTLOG_ERROR_TYPE,TEXT("Winserv: stderr"));
...
static DWORD WINAPI npipes_watchdog(LPVOID clientData)
{
    DWORD wfmo;
	...
	CloseHandle(CreateFile(pipenames[0],FILE_WRITE_DATA|FILE_READ_DATA,0,NULL,
		OPEN_EXISTING, 0, NULL));
	CloseHandle(CreateFile(pipenames[1],FILE_WRITE_DATA|FILE_READ_DATA,0,NULL,
		OPEN_EXISTING, 0, NULL));
...
WSLINKAGE VOID ipc_setup(int after) {
	...
	if (!after) {
	    ...
	    pipenames[0] = str_printf(
		    TEXT("\\\\.\\pipe\\winserv.scm.out.%s"),sv_name);
	    pipenames[1] = str_printf(
		    TEXT("\\\\.\\pipe\\winserv.scm.in.%s"),sv_name);
	    ...
	    npw_events[0]=CreateEvent(NULL,FALSE,FALSE,NULL); // It's ok
...
WSLINKAGE VOID sv_report(LPTSTR msg, int isError)
{
    ...
    for (i=0; msg[i];++i) if (msg[i]==TEXT('\014')) msg[i]=TEXT('\n');
	...
	str_chop(msg);
	eventlog_msg(isError? EVENTLOG_ERROR_TYPE: EVENTLOG_SUCCESS, msg);
    }
}
...
WSLINKAGE VOID sv_handle_escapes(LPTSTR msg)
{
	...
	case TEXT('a'): // ESC a: accept control codes
	    switch (msg[1]) {
		case TEXT('p'):
		    accepts |= SERVICE_ACCEPT_PAUSE_CONTINUE; break;
		case TEXT('c'):
		    accepts |= SERVICE_ACCEPT_PARAMCHANGE; break;
		case TEXT('s'):
		    accepts |= SERVICE_ACCEPT_SHUTDOWN; break;
		case TEXT('n'):
		    accepts |= SERVICE_ACCEPT_NETBINDCHANGE; break;
		case TEXT('r'):
		    accepts = SERVICE_ACCEPT_STOP; break;
		case TEXT('P'):
		    accepts &= ~SERVICE_ACCEPT_PAUSE_CONTINUE; break;
		case TEXT('C'):
		    accepts &= ~SERVICE_ACCEPT_PARAMCHANGE; break;
	...
	case TEXT('s'): // ESC s: set status value
	    switch (msg[1]) {
		case TEXT('p'):
		    sv_status.dwCurrentState = SERVICE_PAUSED; break;
		case TEXT('P'):
		    sv_status.dwCurrentState = SERVICE_PAUSE_PENDING; break;
		case TEXT('C'):
		    sv_status.dwCurrentState = SERVICE_CONTINUE_PENDING; break;
		case TEXT('r'):
		    sv_status.dwCurrentState = SERVICE_RUNNING; break;
		case TEXT('s'):
		    sv_status.dwCurrentState = SERVICE_START_PENDING; break;
		case TEXT('S'):
		    sv_status.dwCurrentState = SERVICE_STOP_PENDING; break;
		default: return;
	    }
	    break;
	case TEXT('e'): // ESC e: log a message with type specified 
	    str_chop(msg);
	    switch (msg[1]) {
		case TEXT('e'): evtype = EVENTLOG_ERROR_TYPE; break;
		case TEXT('w'): evtype = EVENTLOG_WARNING_TYPE; break;
		case TEXT('i'): evtype = EVENTLOG_INFORMATION_TYPE; break;
		case TEXT('s'): evtype = EVENTLOG_SUCCESS; break;
		case TEXT('a'): evtype = EVENTLOG_AUDIT_SUCCESS; break;
		case TEXT('A'): evtype = EVENTLOG_AUDIT_FAILURE; break;
		default: return;
	    }
 ...
 * First we try to send WM_QUIT to the application's thread; if it is still running
 * after 10 sec, TerminateProcess is our friend.
 * */
WSLINKAGE int please_shutdown()
{
...
WSLINKAGE VOID puts_control()
{
    ...
    {
	/* We attempt to shut down the process gracefully
	 * sending WM_CLOSE to all its windows.
	 * */
	if (!please_shutdown())
	    TerminateProcess(pinfo.hProcess,0);
	return;
     ...
     * and there is no communication between winserv and the managed
     ...
     */
    if (ipc_write_stream == NULL) {
        return;
    }

    switch (control_code) {
	case SERVICE_CONTROL_STOP:	    ctlname = TEXT("STOP"); break;
	case SERVICE_CONTROL_INTERROGATE:   ctlname = TEXT("INTERROGATE"); break;
	case SERVICE_CONTROL_PAUSE:	    ctlname = TEXT("PAUSE"); break;
	case SERVICE_CONTROL_CONTINUE:	    ctlname = TEXT("CONTINUE"); break;
	case SERVICE_CONTROL_SHUTDOWN:	    ctlname = TEXT("SHUTDOWN"); break;
	case SERVICE_CONTROL_NETBINDADD:    ctlname = TEXT("NETBINDADD"); break;
	case SERVICE_CONTROL_NETBINDREMOVE: ctlname = TEXT("NETBINDREMOVE"); break;
...
/* 
 * Below is the code that enables WinServ to install, remove and manage 
 * the services.
 * */
...
static LPTSTR util_Keypath=NULL, util_KeypathWs=NULL, util_KeypathEventLog=NULL,
    util_host=NULL;


WSLINKAGE void util_mkregnames()
{
    if (!sv_name) return;

    util_Keypath=str_printf(
	    TEXT("System\\CurrentControlSet\\Services\\%s"),sv_name);
    util_KeypathWs=str_printf(
	    TEXT("System\\CurrentControlSet\\Services\\%s\\WinServ"),sv_name);
    util_KeypathEventLog=str_printf(
	    TEXT("System\\CurrentControlSet\\Services\\eventlog\\Application\\%s"),
	    sv_name);
}

WSLINKAGE void util_decompose_name(LPTSTR name)
{
    int i;
    sv_name = name;
 ...
 * */
WSLINKAGE LPTSTR * util_bind_argv(int org_argc, LPTSTR cmdLine)
{
    LPTSTR p;
    LPTSTR *argv;
...
WSLINKAGE void util_validate_name()
{
    int i;
    if (sv_name) {
	for (i=0;sv_name[i];++i) {
...
WSLINKAGE BOOL util_install(LPTSTR new_cmd_line)
{
    HKEY wskey,svkey,evkey;
    DWORD types_supported = 7;
    LPTSTR ipcs[]={TEXT("blind"),TEXT("pipe"),TEXT("stdio"),TEXT("qstdio")};
	...
	_sntprintf(qsc.lpDisplayName,1023,
		TEXT("%s (managed by WinServ)"),sv_name);
    }
    util_hSvc = CreateService(
	    util_hScm, sv_name,
	    qsc.lpDisplayName,
	    SERVICE_ALL_ACCESS,
    ...
    evkey=reg_open(util_host,HKEY_LOCAL_MACHINE,util_KeypathEventLog,O_CREAT);
    if (evkey) {
	...
	wskey=reg_open(NULL,svkey,TEXT("WinServ"),O_CREAT);
	if (wskey) {
	    DBG(stderr,TEXT("Opened registry wskey\n"));
	    if (new_cmd_line)
		RegSetValueEx(wskey,TEXT("CommandLine"),0,expand?REG_EXPAND_SZ:REG_SZ,
			(LPBYTE)new_cmd_line,_tcslen(new_cmd_line)*sizeof(TCHAR));
	    if (ipc_method!=IPCMETHOD_NO_CHANGE)
		RegSetValueEx(wskey,TEXT("IpcMethod"),0,REG_SZ,
			(LPBYTE)ipcs[ipc_method],
...
WSLINKAGE BOOL util_mustnotbeforeign(LPCTSTR msg)
{
    HKEY wskey=reg_open(util_host,HKEY_LOCAL_MACHINE,util_KeypathWs,O_RDONLY);
    if (!wskey) {
	if (force_foreign) {foreign=TRUE; return TRUE;}
	con_ftprintf(stderr,TEXT("%s: %s is not managed by WinServ.\n\
Use -forceforeign to do it anyway.\n"),msg,sv_name);
	return FALSE;
    } else reg_close(wskey);
    return TRUE;
}

WSLINKAGE QUERY_SERVICE_CONFIG * util_query_config()
{
	...
	free(tqsc); return NULL;
    } else return tqsc;
}

WSLINKAGE BOOL util_writeconf(LPCTSTR new_cmd_line)
{
    HKEY wskey,svkey,evkey;
    BOOL r;
    LPTSTR ipcs[]={TEXT("blind"),TEXT("pipe"),TEXT("stdio"),TEXT("qstdio")};
	...
	if (!util_mustnotbeforeign(TEXT("Won't change -image or command line"))) {
	    SetLastError(ERROR_SUCCESS);
	    return FALSE;
	}
    }
    ...
    /**/
    r=ChangeServiceConfig(util_hSvc,
	    svt,
	    qsc.dwStartType,
	    qsc.dwErrorControl,
	    qsc.lpBinaryPathName,
	...
	evkey=reg_open(util_host,HKEY_LOCAL_MACHINE,util_KeypathEventLog,O_RDWR);
	if (evkey) {
	    RegSetValueEx(evkey,TEXT("EventMessageFile"),0,REG_SZ,
		    (LPBYTE)qsc.lpBinaryPathName,
		    _tcslen(qsc.lpBinaryPathName)*sizeof(TCHAR));
    ...
    if (svkey) {
	DBG(stderr,TEXT("Opened registry svkey\n"));
	wskey=reg_open(NULL,svkey,TEXT("WinServ"),force_foreign?O_CREAT:O_RDWR);
	if (wskey) {
	    DBG(stderr,TEXT("Opened registry wskey\n"));
	    if (new_cmd_line)
		RegSetValueEx(wskey,TEXT("CommandLine"),0,expand?REG_EXPAND_SZ:REG_SZ,
    ...
    if (description)
	DBG(stderr,TEXT("Description is %s\n"),description);
    if (description)
	RegSetValueEx(svkey,TEXT("Description"),0,REG_SZ,
		(LPBYTE)description,
...
WSLINKAGE BOOL util_show()
{
    ...
    /**/
    interactive=((tqsc->dwServiceType&SERVICE_INTERACTIVE_PROCESS)!=0);
    svkey = reg_open(util_host,HKEY_LOCAL_MACHINE,util_Keypath,O_RDONLY);
    if (svkey) {
	wskey = reg_open(NULL,svkey,TEXT("WinServ"),O_RDONLY);
	if (wskey) {
	    reg_read(wskey,TEXT("IpcMethod"),(LPBYTE*)&ipcm,FALSE);
	    kt = reg_read(wskey,TEXT("CommandLine"),(LPBYTE*)&command_line,FALSE);
	    expand = (kt==REG_EXPAND_SZ);
	    ipc_method=lookup_kw(ipcm,kw_ipcmethod);
	    reg_close(wskey);
	} else 
	    foreign = 1;
	reg_read(svkey,TEXT("Description"),(LPBYTE*)&description,FALSE);
	reg_close(svkey);
    }  else {
	reg_unavailable=1;
    }
...
"), tqsc->lpDisplayName, 
   ...
   tqsc->lpServiceStartName);
    if (foreign) {
	con_ftprintf(stdout,
		TEXT("Service %s is not managed by WinServ\n"),sv_name);
    } else {
	if (reg_unavailable) {
	    con_ftprintf(stdout,
		    TEXT("WARNING: Can't open registry, some information is unavailable\n"));
	} else {
...
WSLINKAGE int util_main(int argc, LPTSTR *argv)
{
    ...
    if (!util_hScm) { isOk = FALSE; goto reportError; }
    if (cmd!=CMD_INSTALL) {
	util_hSvc = OpenService(util_hScm,sv_name,asvc);
	if (!util_hSvc) { isOk = FALSE; goto reportError; }
    }
    if (argc && (cmd!=CMD_RESTART) && (cmd!=CMD_START) && (cmd!=CMD_INSTALL)
	    && (cmd!=CMD_CONFIGURE) && (cmd!=CMD_STATUS) && (cmd!=CMD_SHOW)) {
	...
	case CMD_START:
	    ...
	    break;
	case CMD_RESTART: nowait = 0;
	case CMD_STOP:
	    isOk = ControlService(util_hSvc,SERVICE_CONTROL_STOP,&sv_status);
	    if (!isOk) goto reportError;
	    if (!nowait) {
		while (sv_status.dwCurrentState != SERVICE_STOPPED) {
		    Sleep(sv_status.dwWaitHint>1000?1000:sv_status.dwWaitHint);
	    ...
	    break;
	case CMD_PAUSE: 
	    isOk = ControlService(util_hSvc,SERVICE_CONTROL_PAUSE,&sv_status);
	    if (!isOk) goto reportError;
	    if (!nowait) {
	    ...
	    break;
	case CMD_CONTINUE: 
	    isOk = ControlService(util_hSvc,SERVICE_CONTROL_CONTINUE,&sv_status);
	    if (!isOk) goto reportError;
	    if (!nowait) {
	...
	case CMD_STATUS:
	    for (;;) {
		isOk = QueryServiceStatus(util_hSvc,&sv_status);
		if (!isOk)  {isOk=FALSE; goto reportError;}
		...
		if (sv_status.dwControlsAccepted & SERVICE_ACCEPT_STOP)
		    _tprintf(TEXT(" STOP"));
		if (sv_status.dwControlsAccepted & SERVICE_ACCEPT_PAUSE_CONTINUE)
		    _tprintf(TEXT(" PAUSE CONTINUE"));
		if (sv_status.dwControlsAccepted & SERVICE_ACCEPT_SHUTDOWN)
		    _tprintf(TEXT(" SHUTDOWN"));
		if (sv_status.dwControlsAccepted & SERVICE_ACCEPT_PARAMCHANGE)
		    _tprintf(TEXT(" PARAMCHANGE"));
		if (sv_status.dwControlsAccepted & SERVICE_ACCEPT_NETBINDCHANGE)
		    _tprintf(
			TEXT(" NETBINDADD NETBINDREMOVE NETBINDENABLE NETBINDDISABLE"));
		_tprintf(TEXT("\n"));
		if (argc<=0) break;
...
reportError:
	...
	if (err != ERROR_SUCCESS) {
	    err_length = FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | 
		    FORMAT_MESSAGE_FROM_SYSTEM |
		    FORMAT_MESSAGE_IGNORE_INSERTS,
		    NULL, err, LANG_NEUTRAL, (LPTSTR)&error_msg, 0, NULL );
	    if(err_length) {
		con_ftprintf(stderr,TEXT("ERROR %d: %s"),err,error_msg);
	    }
	    return err;
	}
	return 1;
